package org.example.kafka.containers;
...
public class KerberosContainer extends GenericContainer<KerberosContainer> {
	...
	public void createTestPrincipals() {
		try {
			Container.ExecResult lsResult = execInContainer("kadmin.local", "-q", "addprinc -randkey kafka/localhost@EXAMPLE.COM");
			lsResult = execInContainer("kadmin.local", "-q", "ktadd -norandkey -k /tmp/keytab/kafkabroker.keytab kafka/localhost@EXAMPLE.COM");
			lsResult = execInContainer("kadmin.local", "-q", "addprinc -randkey client/localhost@EXAMPLE.COM");
			lsResult = execInContainer("kadmin.local", "-q", "ktadd -norandkey -k /tmp/keytab/client.keytab client/localhost@EXAMPLE.COM");
		} catch (Exception e) {
			e.printStackTrace();
		} 
	}
	...
	public void createKrb5File() {
		...
		try {
			...
			file = new FileOutputStream("src/test/resources/krb5.conf");
